...
otr_name = st.selectbox(
    ...
    ('A01_02, Сельское хозяйство, охота, лесное хозяйство', 
     'B05_06, Горнодобывающая промышленность, производство энергии',
     'B07_08, Горнодобывающая промышленность, неэнергетическая продукция',
     'B09, Деятельность службы поддержки горнодобывающей промышленности',
...
otr_slov = {
    'A01_02, Сельское хозяйство, охота, лесное хозяйство':0, 
     'B05_06, Горнодобывающая промышленность, производство энергии':1,
     'B07_08, Горнодобывающая промышленность, неэнергетическая продукция':2,
     'B09, Деятельность службы поддержки горнодобывающей промышленности':3,
...
if FLAG > 0:
    ...
    g_rus = G_new[n_RUS*35 + otr][cou_to*35 + otr]
    g_rus_old = G_nach[n_RUS*35 + otr][cou_to*35 + otr]

    col1, col2 = st.columns(2)
    with col1:
        st.metric(label="Торговый коэффициент", value= round(g_rus, 4), delta= '{:.1%}'.format(g_rus/g_rus_old - 1))
    with col2:
        st.metric(label="Экспорт продукции "+ ind_agr_rus[otr] + " из RUS в " + cou_to_name, value='{:1.0f} млн.$'.format(exp_rus_to_cou1), delta= '{:.1%} {:1.0f} млн. $'.format(exp_rus_to_cou1/exp_rus_to_cou1_old - 1, exp_rus_to_cou1 - exp_rus_to_cou1_old) )
